...
function displayUploadMessage() {
    // Create a paragraph element for the upload message
    let uploadMessage = document.createElement('p');
    uploadMessage.textContent = 'Images uploaded successfully!';
    uploadMessage.classList.add('upload-message');

    // Append the upload message to the drop area
    dropArea.appendChild(uploadMessage);

    // Remove the upload message after 3 seconds (3000 milliseconds)
    setTimeout(function() {
        dropArea.removeChild(uploadMessage);
    }, 3000);
}